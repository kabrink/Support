...
```{r barplot_medians}
#function that takes in a vector of median values, and num of clusters, a vector of names for those clusters, a vector of names for groups, and a vector of colors for each group and formats them into a clustered bar graph
...
youngsupport = support[which(support$Age<=mean(support$Age)),]
oldsupport = support[which(support$Age>mean(support$Age)),]
...
hist(support$ContactTest1, col = conCol, breaks = 10)
hist(support$ContactTest2, col = conCol, breaks = 10)
hist(support$ContactTest3, col = conCol, breaks = 10)
hist(support$NoContactTest1, col = nocCol, breaks = 10)
hist(support$NoContactTest2, col = nocCol, breaks = 10)
hist(support$NoContactTest3, col = nocCol, breaks = 10)
```
...
For infants younger than mean(support$Age) months, there was no difference in looking times between the first Possible test event (M =
...
For infants younger than mean(support$Age) months, there was no difference in looking times between the second Possible test event (M = 
...
For infants younger than mean(support$Age) months, there was no difference in looking times between the third Possible test event (M =
...
For infants older than mean(support$Age) months, there was no difference in looking times between the first Possible test event (M =
...
For infants older than mean(support$Age) months, there was no difference in looking times between the second Possible test event (M = 
...
For infants older than mean(support$Age) months, there was no difference in looking times between the third Possible test event (M =
...
```{r dishab_hist}
hist(poss.first$dishab, col = conCol, breaks = 10, main = "Dishabituation to first Possible Test Trial")
hist(imp.first$dishab, col = nocCol, breaks = 10, main = "Dishabituation to first Impossible Test Trial")
```
...
##Breakdown of young (<=mean(support$Age) months) vs old infants (>mean(support$Age))
```{r young_vs_old_data}
youngsupport = support[which(support$Age<=mean(support$Age)),]
oldsupport = support[which(support$Age>mean(support$Age)),]
              
pander(table(support$Age<=mean(support$Age), support$Order))
pander(table(support$Age<=mean(support$Age), support$Sex))

pander(table(youngsupport$Sex,youngsupport$Order), caption="Infants younger than mean(support$Age) months")
pander(table(oldsupport$Sex,oldsupport$Order), caption="Infants older than mean(support$Age) months")
```

There were `r length(youngsupport$ContactTest.Average)` less than or equal to mean(support$Age) months old.
There were `r length(oldsupport$ContactTest.Average)` greater than or equal to mean(support$Age) months old.
...
colnames(means) <- c("< mean(support$Age) months","> mean(support$Age) months")
